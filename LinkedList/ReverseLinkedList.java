...
//The line that trips people up is head.next.next = head. Take the call where head = 2. By then 3 is already reversed, but 2.
//next still points to 3, so this line means "make 3 point back to 2." Don't forget head.next = null, or 2 and 3 will point at each other and form a cycle.
public ListNode reverseList(ListNode head) {
...
/*
A singly linked list only goes one way. A node knows its next node but not its previous one. 
The simplest approach is to dump all the values into an array or stack and rebuild the list. That works, but it costs O(n) extra memory and skips the skill this problem is testing: manipulating pointers in place.
